...
| Trigger event | While signed in, on the Artist page click `Trigger LLM Activity`; `POST /api/events` with `artistId` and required `eventType` (for example, `page_activation`) and session cookie. | Status 200; a success toast notification appears in the UI. Signed-out users see the control disabled with a sign-in hint. |
